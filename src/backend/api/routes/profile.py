...
from ..routes.auth import token_required, urlsafe

from ..models import db
from ..models.users import User
from ..models.news import News, _NEWS_SOURCE
...
def profile_data(current_user):
    ...
    if articles:
        ...
        for article in saved_articles:
         # TODO: create the serializer inside the model itself
            articles.append({
...
def delete_preference(current_user):
    ...
    Args:
        current_user (.models.User): current user object as resolved from access token
...
def email_notification(current_user):
    ...
    current_user.email_notify = bool(email_notify)
    db.session.commit()
...
def change_email(current_user):
    ...
    try:
        # again confirmation is needed to be sent for new email
        # token
